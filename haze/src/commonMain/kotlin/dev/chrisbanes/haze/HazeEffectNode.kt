...
import dev.chrisbanes.haze.effect.resolveBlurRadius
...
class HazeEffectNode(
   ...
   *
   * - Globally, if [dev.chrisbanes.haze.effect.invalidateOnHazeAreaPreDraw] is set to true. This is mostly for older
   *   Android versions.
  ...
  private fun updateEffect(): Unit = trace("HazeEffectNode-updateEffect") {
    ...
    val blurRadiusPx = with(currentValueOf(LocalDensity)) {
      ((visualEffect as? BlurVisualEffect)?.resolveBlurRadius() ?: 0.dp).takeOrElse { 0.dp }.toPx()
    }
