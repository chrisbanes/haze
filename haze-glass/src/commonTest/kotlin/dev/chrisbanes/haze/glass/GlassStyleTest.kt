...
class GlassStyleTest {
  ...
  fun directOptics_usesFixedValidation() {
    ...
    assertFixedAndDirectOpticsFailure(
      ...
      fixed = { GlassOptics(depth = GlassOptics.SizeValue.Fixed(Float.NEGATIVE_INFINITY)) },
      direct = { optics(depth = Float.NEGATIVE_INFINITY) },
    )
    assertFixedAndDirectOpticsFailure(
      "blurRadius must be specified, finite, and non-negative",
      fixed = { GlassOptics(blurRadius = GlassOptics.SizeValue.Fixed((-1).dp)) },
  ...
  fun completeOptics_preservesAdaptiveAndProgrammaticValues() {
    val sharedFixed = GlassOptics(depth = GlassOptics.SizeValue.Fixed(0.4f))
  ...
  fun then_composesRecordedStylesWithoutRerunningBuilders() {
    ...
    assertThat(first.optics).isEqualTo(GlassOptics(depth = GlassOptics.SizeValue.Fixed(0.6f)))
  ...
  fun builtInStyles_replaceEveryMaterialResponseWithoutReplacingPresentation() {
    ...
    val local = GlassStyle {
      ...
      optics(GlassOptics(depth = GlassOptics.SizeValue.Fixed(0.9f), blurRadius = GlassOptics.SizeValue.Fixed(30.dp)))
  ...
  fun builtInStyle_allowsLaterCustomMaterialWritesToWin() {
    val fixedOptics = GlassOptics(depth = GlassOptics.SizeValue.Fixed(0.7f), blurRadius = GlassOptics.SizeValue.Fixed(24.dp))
    val resolved = resolveGlassStyleValues(
      GlassStyle,
