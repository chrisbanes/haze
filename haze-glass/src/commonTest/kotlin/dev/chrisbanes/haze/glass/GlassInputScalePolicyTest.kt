...
class GlassInputScalePolicyTest {
  ...
  fun fixedProfiles_areMonotonicAcrossAscendingQualityFractions() {
    ...
    assertThat(profiles).isEqualTo(listOf(0.5f, sqrt(0.325f), sqrt(0.4375f), sqrt(0.625f), sqrt(0.8125f), 1f))
  }

  @Test
  fun adaptive_withoutHostTimingUsesMiddleTier() {
    assertThat(GlassInputScalePolicy().resolve(HazePerformanceMode.Adaptive))
      .isEqualTo(GlassInputScalePolicy.BALANCED_SCALE)
  }

  @Test
  fun fixedProfilesRemainIndependentOfAdaptiveFallback() {
    val policy = GlassInputScalePolicy()

    assertThat(policy.resolve(HazePerformanceMode.Adaptive))
      .isEqualTo(GlassInputScalePolicy.BALANCED_SCALE)
    assertThat(policy.resolve(HazePerformanceMode.Quality)).isEqualTo(1f)
    assertThat(policy.resolve(HazePerformanceMode.Performance)).isEqualTo(0.5f)
    assertThat(policy.resolve(HazePerformanceMode.Adaptive))
  ...
  fun adaptiveAndEquivalentFixedFractionResolveSameProfile() {
    val adaptive = GlassInputScalePolicy().resolve(HazePerformanceMode.Adaptive)

    assertThat(adaptive).isEqualTo(
      GlassInputScalePolicy().resolve(HazePerformanceMode.Fixed(1f / 3f)),
    )
  }
}
