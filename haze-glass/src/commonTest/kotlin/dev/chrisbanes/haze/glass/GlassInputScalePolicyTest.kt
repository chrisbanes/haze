...
class GlassInputScalePolicyTest {
  ...
  fun fixedQuality_interpolatesTotalPixelsAcrossSupportedRange() {
    for (step in 0..100) {
      ...
      val scale = HazePerformanceMode.Fixed(quality).resolveGlassInputScale()
  ...
  fun namedFixedModes_resolveValidatedProfiles() {
    assertThat(HazePerformanceMode.Quality.resolveGlassInputScale()).isEqualTo(1f)
    assertThat(HazePerformanceMode.Balanced.resolveGlassInputScale())
      .isCloseTo(sqrt(0.625f), 0.0001f)
    assertThat(HazePerformanceMode.Performance.resolveGlassInputScale()).isEqualTo(0.5f)
  }

  @Test
  fun fixedProfiles_areMonotonicAcrossAscendingQualityFractions() {
    val profiles = listOf(0f, 0.1f, 0.25f, 0.5f, 0.75f, 1f)
      .map { HazePerformanceMode.Fixed(it).resolveGlassInputScale() }
  ...
  fun adaptiveCompatibilityAlias_usesBalancedProfile() {
    assertThat(HazePerformanceMode.Adaptive.resolveGlassInputScale())
