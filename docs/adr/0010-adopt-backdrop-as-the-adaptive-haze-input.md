...
When the native path is unavailable, Haze uses `Backdrop`'s `fallback` source input when one is
supplied. `HazeInput.Backdrop()` has no portable source fallback: content remains unchanged and
Haze does not demand source capture. A supplied fallback's selection and retention policies apply
only to that source-capture path; they do not filter native window pixels.
...
and setup or draw failures use a supplied source fallback. Without a fallback, content remains
unchanged and Haze does not demand source capture. A known unavailable or failed native path
remains sticky until that modifier node detaches. Existing attached nodes keep the value observed
at attachment; a later attachment reads the current value.
...
transition may take one frame when a fallback is supplied; that fallback remains responsible for
portable rendering and retained-output behavior.
[ADR-0009](0009-use-opt-in-android-window-backdrops.md) records the earlier wrapper-based decision
and remains unchanged as historical context; this ADR supersedes its public input shape.
