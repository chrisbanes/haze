...
`Adaptive` and `Fixed`. Its blur radius and depth each own an independent `GlassOptics.SizeValue<T>`:
either a `Fixed` value or an `Interpolated` value containing two or more
`GlassOptics.SizePoint<T>` values. Each point pairs a strictly increasing shortest dimension with one
...
runtime distinctions. Interpolated-value lists are snapshotted and rejected unless they contain at
