# Android Glass Depth Spike Design

## Context

Issue #1009 tracks a Glass rendering limitation on Android 13+. Skiko can
...
composition strategy: draw a blurred underlay first, then draw a Glass
overlay from the original content layer.

## Goal

Prove whether an Android two-pass draw can provide acceptable Glass
...
- Do not change public Glass APIs.
...
`RuntimeShaderGlassDelegate` already records:

1. Set the layer's render effect to a blurred underlay effect.
2. Draw the layer.
3. Set the layer's render effect to a Glass overlay effect.
...
should use `GlassShaders.ContentMode.OverlayWithExternalUnderlay`, sampling
...
- `GlassRenderEffectFactory.skiko.kt` continues to use the dual-input
...
- Replace the current single Android `SingleBlurredInput` effect with two
  ...
  - an overlay Glass runtime shader effect using
...
Glass screenshot helpers are sufficient; this should be a visual contract
...
- The extra pass is obviously too expensive for common Glass use.
